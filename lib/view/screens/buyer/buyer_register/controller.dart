...
class BuyerRegisterController {
    ...
    if (kIsWeb) {
      ...
      try {
        ConfirmationResult confirmationResult = await FirebaseAuth.instance
            .signInWithPhoneNumber("+91" + data["contactNo"]);
        vid = confirmationResult.verificationId;
        EasyLoading.dismiss();
        data["vid"] = vid;
        Navigator.pushNamed(context, RoutePaths.verifyBuyer, arguments: data);
      } catch (e) {
  ...
  Future<void> verifyPhoneNumber(
      BuildContext context, var data, String otp) async {
    EasyLoading.show(
      status: "Verifying",
      dismissOnTap: false,
    );
    if (otp.length < 6) {
      ...
      PhoneAuthCredential phoneAuthCredential = PhoneAuthProvider.credential(
          verificationId: data["vid"], smsCode: otp);
      UserCredential? user;
      try {
      ...
      if (user != null) {
        try {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text("register " + e.toString())));
        }
      }
    }
  }
