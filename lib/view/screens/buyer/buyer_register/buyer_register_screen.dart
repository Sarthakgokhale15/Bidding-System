...
class BuyerRegisterScreen extends StatefulWidget {
  ...
  @override
  _BuyerRegisterScreenState createState() => _BuyerRegisterScreenState();
}
...
class _BuyerRegisterScreenState extends State<BuyerRegisterScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar:
            AppBarHelper.getSimpleAppBar(name: "Register", context: context),
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextInputController.simpleTextInputController(
                      context, "Full Name", nameCtrl),
                  TextInputController.emailTextInputController(
                      context, 'Email', emailCtrl),
                  TextInputController.phoneInputCtrl(
                      context, "Phone Number", phoneCtrl),
                  TextInputController.aadharInputCtrl(
                      context, "Aadhar Card Number", aadharCtrl),
                  TextInputController.simpleTextInputController(
                      context, "Registered Company Name", cmpNameCtrl),
                  TextInputController.simpleTextInputController(
                      context, "Address Line 1", addressCtrl1),
                  TextInputController.simpleTextInputController(
                      context, "Address Line 2", addressCtrl2),
                  stateAndCityPicker(),
                  TextInputController.simpleTextInputController(
                      context, "Role In Company", roleCtrl),
                  TextInputController.simpleTextInputController(
                      context, "Company Pan", cmpPanCtrl),
                  TextInputController.simpleTextInputController(
                      context, "Company GSTIN", cmpGstCtrl),
                  Container(
  ...
  Future<void> gotoVerify() async {
    if (nameCtrl.text == "") {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text("Enter gst")));
    } else if (cmpNameCtrl.text == "") {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text("Enter Company Name")));
    } else if (await checkUserPresent(phoneCtrl.text)) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text("User Already Present")));
    } else {
      var data = {
        "name": nameCtrl.text,
      ...
      BuyerRegisterController buyerRegisterController =
  ...
  Future<bool> checkUserPresent(String phone) async {
    ...
    bool x = await loginBuyerToFirebase(
      contactNo: "+91" + phone,
    );
    EasyLoading.dismiss();
    if (x) {
